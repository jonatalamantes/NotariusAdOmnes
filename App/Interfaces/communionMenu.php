<?php 
    ...
    SessionManager::validateUserInPage('communionMenu.php');

    //Get File contest from template
    $string = file_get_contents("template/CommunionMenu.html");
    ...
    if ($communionRegistries !== NULL && $totalRegistries != 0)
    {
        $table = '<table class="col-md-12 table-condensed table-bordered cf menuTable" id="table1">';

        $header = '<thead class="cf">
                    <tr>
                        <th data-title="" colspan="4" class="center"><strong>^Communion Table^</strong></th>
                        <th data-title="Sort" style="text-align: center" colspan="2" class="center">
                            ^Sort By^ 
                            <select class="form-control" id="sortType" onchange="loadSort(\'communionMenu.php\')">
        ...
        $i = 0;
        foreach ($communionRegistries as $singleCommunion) 
        {
            if ($i === 10) 
            {
                continue;
